...
export class GamePage {
  ...
  ionViewDidLoad() {
    runGame(GAME_LEVELS, DOMDisplay, this.content);
...
function Level(plan) {
  ...
  for(var y=0 ; y<this.height ; y++) {
    ...
    for(var x=0 ; x<this.width ; x++) {
      var ch = line[x];
      var fieldType = null;
      var Actor = actorChars[ch];     /// aca falta algo
      if(Actor)
...
DOMDisplay.prototype.scrollPlayerIntoView = function() {
  ...
  var center = player.pos.plus(player.size.times(0.5)).times(scale); // mas la mitad del cuerpecito, mas la escala. centro centro.
...
function runGame(plans, Display, content) {
  function startLevel(n) {
    runLevel(new Level(plans[n]), Display, content, function(status) {
      if(status == "lost")
        startLevel(n);
